...
const AdminSignup = () => {
 ...
 const[adminKey,setAdminKey]=useState({key:''})       
    ...
    const handleSubmit = (event) =>
    {
        event.preventDefault();
         ...
         const err = (Validation(admin,adminKey.key));
        setErrors(err);
    
        //Checking validations
        if(err.name==="" && err.email==="" && err.password==="" && adminKey.key==="adm234")
        {
            //Storing students information to the database
            axios.post('http://localhost:3001/admin_signup',admin)
            .then(result=>{if(result.data.Status){   
                alert("Successfully Signed up Admin in the system.")
                navigate('/')    }
  ...
  return (
    ...
    <h2><i className='fs-1 bi-person  d-flex justify-content-center align-item-center'></i></h2>
                <h3 className='d-flex justify-content-center align-item-center'>Admin Sign Up</h3>
        <form action='' onSubmit={handleSubmit}>
            ...
            <div className='mb-3'>
    <label htmlFor='key'><strong>Enter Key: </strong></label>
    <input
        type='key'
        placeholder='Enter Key'
        name='key'
        required   value={adminKey.key}
        onChange={(e) => setAdminKey({ ...adminKey, key: e.target.value })}
        className='form-control rounded-0'
    />
